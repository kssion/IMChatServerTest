...
<html>
 ...
 <body>
   ...
   <p>
    For example, to add the <tt>socket-gui</tt> role to a user named
    <tt>tomcat</tt> with a password of <tt>s3cret</tt>, add the following to the
    config file listed above.
   </p>
<pre>
&lt;role rolename="socket-gui"/&gt;
&lt;user username="tomcat" password="s3cret" roles="socket-gui"/&gt;
</pre>
   <p>
    Note that for Tomcat 7 onwards, the roles required to use the socket
    application were changed from the single <tt>socket</tt> role to the
    following four roles. You will need to assign the role(s) required for
    the functionality you wish to access.
   </p>
    <ul>
      <li><tt>socket-gui</tt> - allows access to the HTML GUI and the status
          pages</li>
      <li><tt>socket-script</tt> - allows access to the text interface and the
          status pages</li>
      <li><tt>socket-jmx</tt> - allows access to the JMX proxy and the status
          pages</li>
      <li><tt>socket-status</tt> - allows access to the status pages only</li>
    </ul>
   <p>
   ...
   <ul>
    <li>Users with the <tt>socket-gui</tt> role should not be granted either
        the <tt>socket-script</tt> or <tt>socket-jmx</tt> roles.</li>
